...
/*
* @FileName 		 : Sales_delivery.php
* @Class  			 : Sales_delivery
* Model Name         : Sales_delivery
* Description        :
* Module             : company
* Actors 	         : -
...
class Sales_delivery extends MY_Controller
{
	...
	{
		parent::__construct();
		$this->config->load('table_config/tbl_sales_delivery.php');
		$this->config->load('table_config/tbl_sales_delivery_items.php');
		$this->load->model('company/sales_delivery_model', 'nativeModel');
		
		$screenDetails				 = $this->config->item('SCREEN_NAMES')['SALES_DELIVERY'];
	...
	*
	* @DESC 		: TO SAVE THE DELIVEY
	* @RETURN VALUE : $outputdata array
	* @PARAMETER 	: -
	* @SERVICE      : WEB
	* @ACCESS POINT : -
	**/
	public function saveSalesDelivery()
	{
		$this->checkRequestMethod("post"); // Check the Request Method
		$outputData['status']  = "FAILURE";
		$getData 		       = $this->currentRequestData;

		$modelOutput 	  	   = $this->nativeModel->saveSalesDelivery($getData);

		if (1 == $modelOutput['flag']) {
			$outputData['sId']      	= $modelOutput['sId'];
			$outputData['status']       = "SUCCESS";
			$outputData['message']      = lang('MSG_242');  // Successfully Inserted
	...
	*
	* @DESC 		: TO UPDATE THE DELIVEY
	* @RETURN VALUE : $outputdata array
	* @PARAMETER 	: -
	* @SERVICE      : WEB
	* @ACCESS POINT : -
	**/
    public function updateSalesDelivery()
    {
        $this->checkRequestMethod("put"); // Check the Request Method
		$outputData['status']  = "FAILURE";
		$modelOutput		   = $this->nativeModel->updateSalesDelivery($this->currentRequestData);
		
		if (1 == $modelOutput['flag']) {
			$outputData['status']       = "SUCCESS";
			$outputData['message']      = lang('MSG_243'); //'Successfully updated
		}else if (2 == $modelOutput['flag']) {
	 ...
	 * @ACCESS POINT 	: -
	 **/
	public function copySalesDelivery()
	{
		$outputData['status']  = "FAILURE";
		$this->checkRequestMethod("put"); 	// CHECK THE REQUEST METHOD
		...
		if(count($copiedIds) == 1) {
			return $this->editSalesDelivery($copiedIds[0]);
		}else if(count($copiedIds)>1){
			
			$getFirstCopyRecordDetails = $this->nativeModel->editSalesDelivery($copiedIds[0]);
			
			if(count($getFirstCopyRecordDetails) > 0){
				$frameEditDetails = $this->frameSalesDeliveryEditDetails($getFirstCopyRecordDetails[0]);
	...
	public function editSalesDelivery($id = '')
	{
		...
		if (!empty($id) && is_numeric($id)) {
			...
			$modelOutput 				= $this->nativeModel->editSalesDelivery($id);
			...
			if (count($modelOutput) > 0) {
				...
				$frameEditDetails = $this->frameSalesDeliveryEditDetails($modelOutput[0]);
	...
	/**
	* @METHOD NAME 	: frameSalesDeliveryEditDetails()
	*
	* @DESC 		: -
	* @RETURN VALUE : $outputdata array
	* @PARAMETER 	: -
	...
	* @ACCESS POINT : -
	**/
	public function frameSalesDeliveryEditDetails($modelOutput)
    {
	...
	*
	* @DESC 		: TO GET THE SALES DELIVERY LIST  
	* @RETURN VALUE : $outputdata array
	* @PARAMETER 	: -
	* @SERVICE      : WEB
	* @ACCESS POINT : -
	**/
    public function getSalesDeliveryList()
    {
        $this->checkRequestMethod("put"); // Check the Request Method
        $modelOutput           = $this->nativeModel->getSalesDeliveryList($this->currentRequestData);
	...
	public function getAnalyticsDetails()
	{
		...
		$passData['tableName']	= SALES_DELIVERY; 
	...
	{
		$modelOutput	= $this->nativeModel->getSalesDeliveryList($this->currentRequestData, 1);
		$resultsData 	= $modelOutput['searchResults'];
		$fileName		= $this->config->item('SALES_DELIVERY')['excel_file_name'];

		$outputData 	= processExcelData($resultsData, $fileName, $this->config->item('SALES_DELIVERY')['columns_list']);
		$this->output->sendResponse($outputData);
	}
	...
	public function downloadInvoice($id = '', $isMailDoc = 0)
	{
		...
		if (count($modelOutput) > 0) {
			$modelOutput 	 = $this->frameSalesDeliveryEditDetails($modelOutput[0]);
			...
			if (count($companyDetails) == 1) {
				...
				$invoiceProcessData['fileName']			= 'sales_delivery';
				$invoiceProcessData['fileHeadingName']  = 'SALES DELIVERY';
	...
	**/
	...
	public function proceedApprovalActivityForDocument()
	{
		...
		if (!empty($id) && is_numeric($id)) {
			...
			$getDocumentData 			= $this->nativeModel->editSalesDelivery($id);
