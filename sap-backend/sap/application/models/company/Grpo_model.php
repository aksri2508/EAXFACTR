...
* Features           : 
*/
class Grpo_model extends CI_Model
{
	public function __construct()
	{
		parent::__construct();
		$this->tableNameStr = 'GRPO';
		$this->itemTableNameStr = 'GRPO_ITEMS';
		$this->itemTableColumnRef = 'grpo_id';
		$this->itemTableColumnReqRef = 'grpoId';
	 ...
	 * @ACCESS POINT : -
	 **/
	public function saveGrpo($getPostData)
	{
		...
		if ($this->app_db->trans_status() === FALSE) {
			...
			$this->updateDistributionRulesInGrpo($insertId);
		}
		return $modelOutput;
	}
	 ...
	 * @ACCESS POINT 	: -
	 **/
	public function saveGrpoItems($getPostData)
	{
	...
	/**
	 * @METHOD NAME 	: updateDistributionRulesInGrpo()
	 *
	 * @DESC 			: TO UPDATE THE DISTRIBUTION RULES IN PARENT TABLE: GRPO
	 * @RETURN VALUE 	: $modelOutput array
	 * @PARAMETER 		: $getPostData array
	 * @SERVICE 		: WEB
	 * @ACCESS POINT 	: -
	 **/
	public function updateDistributionRulesInGrpo($mainTableId)
	{
	 ...
	 *
	 * @DESC 		 	: TO UPDATE THE GRPO DETAILS
	 * @RETURN VALUE 	: $modelOutput array
	 * @PARAMETER 	 	: $getPostData array
	 * @SERVICE 	 	: WEB
	 * @ACCESS POINT 	: -
	 **/
	public function updateGrpo($getPostData)
	{
		...
		$oldMainDetails  	 			  = $this->editGrpo($id);
		$oldItemDetails['itemListArray']  = $this->editGrpoItemList($id);
		...
		if (count($deletedItemChildIds) > 0) { // Child values
			foreach ($deletedItemChildIds as $key => $value) {
				...
				$this->deleteGrpoItems($passStageId);
			}
		}
		...
		if ($this->app_db->trans_status() === FALSE) {
			...
			$this->updateDistributionRulesInGrpo($id);
		}
	 ...
	 * @ACCESS POINT  : -
	 **/
	public function updateGrpoItems($getPostData)
	{
	...
	/**
	 * @METHOD NAME   : editGrpoItemList()
	 *
	 * @DESC 		  : TO EDIT THE GRPO ITEM LIST. 
	 * @RETURN VALUE  : $rs array
	 * @PARAMETER 	  : $getPostData array
	 * @SERVICE 	  : WEB
	 * @ACCESS POINT  : -
	 **/
	public function editGrpoItemList($id)
	{
	 ...
	 * @ACCESS POINT 	: -
	 **/
	public function deleteGrpoItems($getPostData)
	{
	 ...
	 *
	 * @DESC 			: TO GET THE GRPO LIST
	 * @RETURN VALUE 	: $modelData array
	 * @PARAMETER 		: $getPostData array
	 * @SERVICE 		: WEB
	 * @ACCESS POINT 	: -
	 **/
	public function getGrpoList($getPostData, $downloadFlag = '')
	{
			...
			foreach ($filters as $key => $value) {
				...
				if ($fieldValue!="") {
					if ($fieldName == "vendorBpName") {
						...
						$query .= ' AND DATE(created2_on) <= "' . $fieldValue . '"';
		...
		if (isset($tableProperties['sortField'])) {
			...
			$fieldName	   = getListingParams($this->config->item('GRPO')['columns_list'], $fieldName);
	...
	/**
	 * @METHOD NAME 	: getGrpoItemList()
	 *
	 * @DESC 			: TO GET THE GRPO ITEM LIST 
	 * @RETURN VALUE 	: $rs array
	 * @PARAMETER 		: $getPostData array
	 * @SERVICE 		: WEB
	 * @ACCESS POINT 	: -
	 **/
	public function getGrpoItemList($id)
	{
		$rowData = bindConfigTableValues($this->itemTableNameStr, 'EDIT', $id);
		$this->app_db->select($rowData);
		$this->app_db->from($this->itemTableName);
