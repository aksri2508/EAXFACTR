...
/*
* @FileName 		 : Sales_transcations_model.php
* @Class  			 : Sales_transcations_model
* Model Name         : Sales_transcations_model
* Description        :
* Module             : company
* Actors 	         : -
...
* Features           : 
*/
class Sales_transcations_model extends CI_Model
{
	private $controllerNameRef;
	public function __construct()
	{
		...
		$this->controllerNameRef = $controllerName;

		$screenDetails		= $this->config->item('SCREEN_NAMES')[strtoupper($controllerName)];

		//echo "<pre>";print_r($screenDetails);echo "</pre>";
	 ...
	 * @ACCESS POINT : -
	 **/
	public function saveSalesTransactions($getPostData)
	{
		...
		// AR DP INVOICE 
		if ($this->tableNameStr == 'SALES_AR_DP_INVOICE') {
			$rowData['remaining_amount'] = $getPostData['totalAmount'] - $getPostData['taxPercentage']; // Remaining without Tax 
		}

		// AR-INVOICE AND AR-CREDIT MEMO 
		if (($this->tableNameStr == 'SALES_AR_INVOICE') || ($this->tableNameStr == 'SALES_AR_CREDIT_MEMO')) {
			if ($getPostData['totalAmount'] == 0) {
		...
		if (($this->tableNameStr == 'SALES_AR_INVOICE')) {
			if ($getPostData['salesArDpInvoiceId'] != '') {
				$rowData['sales_ar_dp_invoice_used_remaining_amount'] = $getPostData['salesArDpInvoiceUsedAmount'];
			}
		}
		...
		// UPDATE THE ITEM LAST SALES PRICE & LAST PURCHASE PRICE : THIS IS APPLICABLE FOR SALES AR INVOICE ALONE 
		updateLastPriceInItemTable($getListData, $this->tableNameStr,$documentProcessMode);


		// DIRECTLY CLOSE THE STATUS FOR SALES AR CREDIT MEMO (ONCE COPY FROM SALES AR INVOICE / SALES AR CREDIT MEMO)
		if ($this->tableNameStr == 'SALES_AR_CREDIT_MEMO') {
			directlyCloseStatus($this->tableNameStr, $getListData, $insertId,$documentProcessMode);
		} else if ($this->tableNameStr == 'SALES_RETURN') { // DIRECTLY CLOSE THE STATUS WHILE COPYING FROM & TO OF SALES DELIVERY
			directlyCloseStatus($this->tableNameStr, $getListData, $insertId,$documentProcessMode);
		}
	 ...
	 * @ACCESS POINT 	: -
	 **/
	public function updateSalesTransactions($getPostData)
	{
		...
		$oldMainDetails  	 			  = $this->editSalesTransactions($id);
	...
	/**
	 * @METHOD NAME   : getItemListDetails()
	 *
	 * @DESC 		  : - 
	 * @RETURN VALUE  : $rs array
	 * @PARAMETER 	  : $getPostData array
	 ...
	 * @ACCESS POINT  : -
	 **/
	public function getItemListDetails($id)
	{
		$rowData = bindConfigTableValues($this->itemTableNameStr, 'EDIT', $id);

		foreach ($rowData as $rkey => $rval) {
			$rowData[$rkey] = $this->itemTableName . '.' . $rval;
		}

		array_push($rowData, "bin_id");

		$this->app_db->select($rowData);
		$this->app_db->from($this->itemTableName);
		$this->app_db->join('tbl_item_warehouse', $this->itemTableName . '.item_id = tbl_item_warehouse.item_id', 'left');
		$this->app_db->where($this->itemTableColumnRef, $id);
		$this->app_db->where($this->itemTableName . '.is_deleted', '0');
		$rs = $this->app_db->get();
		return $rs->result_array();
	}
	 ...
	 * @ACCESS POINT 	: -
	 **/
	public function getSalesTransactionsList($getPostData, $downloadFlag = '')
	{
		...
		$salesArInvoiceSpecificFields = '';
		if($this->controllerNameRef == "sales_ar_invoice"){
			$salesArInvoiceSpecificFields = '
			a.tracking_number,
			a.courier,
			a.delivery_status,';
			...
			foreach ($filters as $key => $value) {
				...
				if ($fieldValue!="") {
					if ($fieldName == "customerBpName") {
						...
						$query .= ' AND LCASE(tracking_number) REGEXP LCASE(replace("' . strtolower($fieldValue) . '"," ","|"))';
					} else if ($fieldName == "courier") {
						$query .= ' AND LCASE(courier) REGEXP LCASE(replace("' . strtolower($fieldValue) . '"," ","|"))';
					} else if ($fieldName == "deliveryStatus") {
						$query .= ' AND delivery_status = "' . $fieldValue . '"';
		...
		$passType['type'] = 'SALES_TRANS_STATUS';
		...
		$this->app_db->trans_start();
			...
			if($modelOutput['flag'] == 1) { // Successfully converted 
				...
				if (($this->tableNameStr == 'SALES_AR_INVOICE') || ($this->tableNameStr == 'SALES_AR_CREDIT_MEMO')) {
					...
					if ($getPostData['total_amount'] == 0) {
							$rowData['status'] = 2;  // CLOSE THE DOCUMENT ONCE CLICKED THE ADD BUTTON
							$whereQry		   =  array('id' => $id);
							$this->commonModel->updateQry($this->tableName, $rowData, $whereQry);		
					}
				}
				...
				// DIRECTLY CLOSE THE STATUS FOR SALES AR CREDIT MEMO (ONCE COPY FROM SALES AR INVOICE / SALES AR CREDIT MEMO)
				$getListData = $oldItemDetails['itemListArray'];
				if ($this->tableNameStr == 'SALES_AR_CREDIT_MEMO') {
					directlyCloseStatus($this->tableNameStr, $getListData, $id,$documentProcessMode);
				} else if ($this->tableNameStr == 'SALES_RETURN') { // DIRECTLY CLOSE THE STATUS WHILE COPYING FROM & TO OF SALES DELIVERY
					directlyCloseStatus($this->tableNameStr, $getListData, $id,$documentProcessMode);
				}
