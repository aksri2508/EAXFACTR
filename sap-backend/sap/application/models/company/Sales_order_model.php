...
/*
* @FileName 		 : sales_order_model.php
* @Class  			 : sales_order_model
* Model Name         : sales_order_model
* Description        :
* Module             : company
* Actors 	         : -
...
class Sales_order_model extends CI_Model
{
	...
	{
		parent::__construct();
		$this->tableNameStr 		 = 'SALES_ORDER';
		$this->itemTableNameStr		 = 'SALES_ORDER_ITEMS';
		$this->itemMasterTableNameStr = 'MASTER_ITEM';
		$this->itemTableColumnRef 	 = 'sales_order_id';
		$this->itemTableColumnReqRef = 'salesOrderId';
		$this->tableName 			 =  constant($this->tableNameStr);
		$this->itemTableName 		 =  constant($this->itemTableNameStr);
		$this->itemMasterTableName 			 =  constant($this->itemMasterTableNameStr);
	 ...
	 * @ACCESS POINT : -
	 **/
	public function saveSalesOrder($getPostData)
	{
	 ...
	 * @ACCESS POINT 	: -
	 **/
	public function updateSalesOrder($getPostData)
	{
		...
		$oldMainDetails  	 			  = $this->editSalesOrder($id); 
	 ...
	 * @ACCESS POINT 	: -
	 **/
	public function getSalesOrderList($getPostData, $downloadFlag = '')
	{
	 ...
	 * @ACCESS POINT 	: -
	 **/
	public function getOpenSalesOrdersByItemId($getPostData, $downloadFlag = '')
	{
		// GET THE EMPLOYEE DISTRIBUTION LIST 
		$empDetails  = $this->commonModel->getProfileInformation($this->currentUserId);
		$empDistributionRulesId = $empDetails['profileInfo'][0]['distribution_rules_id'];
	 ...
	 *
	 * @DESC 			: TO GET THE SALES ORDER ITEM LIST DETAILS. 
	 * @RETURN VALUE 	: $rs array
	 * @PARAMETER 		: $getPostData array
	 * @SERVICE 		: WEB
	 * @ACCESS POINT 	: -
	 **/
	public function getItemListDetails($id, $filterVal)
	{	
		$query = 'SELECT * FROM 
					(SELECT
					...
					a.unit_price,
					a.discount_percentage,
					a.tax_id as sales_order_items_tax_id,
					a.hsn_id as sales_order_items_hsn_id,
					a.bin_id,
					a.last_price,item_tax_percentage,
					a.item_tax_value,
					a.total_item_amount,warehouse_id,
					a.distribution_rules_id,
					a.serial_number,
					a.expiry_date,
					a.bp_approval,
					a.remarks as sales_order_items_remarks,
					a.dealer_discount_percentage,
					a.sap_id as sales_order_items_sap_id,
					a.posting_status as sales_order_items_posting_status,
					a.sap_error as sales_order_items_sap_error,
					 ...
					 mi.item_name,
					 mi.item_group_id,
					 mi.uom_id as master_item_uom_id,
					 mi.status,
					 mi.item_image,
					 mi.stock,
					 mi.hsn_id as master_item_hsn_id,
					 mi.manufacturer_id,
					 mi.remarks as master_item_remarks,
					 mi.foreign_name,
					 mi.item_transaction_type,
					 mi.last_purchase_price,
					 mi.last_sales_price,
					 mi.last_price_list_id,
					 mi.sap_id as master_item_sap_id,
					 mi.posting_status as master_item_posting_status,
					 mi.item_weight,
					...
					FROM ' . $this->itemTableName . ' as a	
					
					LEFT JOIN '.MASTER_ITEM.' as mi
						ON mi.id = a.item_id
				...
				$query .= ' AND '.$this->itemTableColumnRef.' = "'. $id . '"';

				
				if(isset($filterVal['itemId']) && $filterVal['itemId'] != ""){
					$query .= ' AND a.item_id = "' . $filterVal['itemId'] . '"';
				}
