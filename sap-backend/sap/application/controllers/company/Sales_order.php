...
/*
* @FileName 		 : Sales_order.php
* @Class  			 : Sales_order
* Model Name         : Sales_order
* Description        :
* Module             : company
* Actors 	         : -
...
class Sales_order extends MY_Controller
{
	...
	{
		parent::__construct();
		$this->config->load('table_config/tbl_sales_order.php');
		$this->config->load('table_config/tbl_sales_order_items.php');
		$this->load->model('company/sales_order_model', 'nativeModel');
		
		
		$screenDetails				 = $this->config->item('SCREEN_NAMES')['SALES_ORDER'];
	...
	**/
	public function saveSalesOrder()
	{
		...
		$modelOutput 	  	   = $this->nativeModel->saveSalesOrder($getData);

		if (1 == $modelOutput['flag']) {
			$outputData['sId']      	= $modelOutput['sId'];
			$outputData['status']       = "SUCCESS";
			$outputData['message']      = lang('MSG_37');  // Successfully Inserted
	...
	*
	* @DESC 		: TO UPDATE THE SALES ORDER 
	* @RETURN VALUE : $outputdata array
	* @PARAMETER 	: -
	* @SERVICE      : WEB
	* @ACCESS POINT : -
	**/
    public function updateSalesOrder()
    {
		...
		$outputData['status']  = "FAILURE";
		$modelOutput		   = $this->nativeModel->updateSalesOrder($this->currentRequestData);
		
		if (1 == $modelOutput['flag']) {
			$outputData['status']       = "SUCCESS";
			$outputData['message']      = lang('MSG_39'); //'Successfully updated
		}else if (2 == $modelOutput['flag']) {
	 ...
	 * @ACCESS POINT 	: -
	 **/
	public function copySalesOrder()
	{
		$outputData['status']  = "FAILURE";
		$this->checkRequestMethod("put"); 	// CHECK THE REQUEST METHOD
		...
		if(count($copiedIds) == 1) {
			return $this->editSalesOrder($copiedIds[0]);
		}else if(count($copiedIds)>1){
			
			$getFirstCopyRecordDetails = $this->nativeModel->editSalesOrder($copiedIds[0]);
			
			if(count($getFirstCopyRecordDetails) > 0){
				$frameEditDetails = $this->frameSalesOrderEditDetails($getFirstCopyRecordDetails[0]);
	...
	/**
	 * @METHOD NAME 	: editSalesOrder()
	 *
	 * @DESC 			: TO EDIT SALES ORDER
	 * @RETURN VALUE 	: $outputdata array
	 * @PARAMETER 		: -
	 * @SERVICE      	: WEB
	 * @ACCESS POINT 	: -
	...
	public function editSalesOrder($id = '')
	{
		...
		if (!empty($id) && is_numeric($id)) {
			...
			$modelOutput 				= $this->nativeModel->editSalesOrder($id);
			...
			if (count($modelOutput) > 0) {
				...
				$frameEditDetails = $this->frameSalesOrderEditDetails($modelOutput[0]);
	...
	/**
	* @METHOD NAME 	: frameSalesOrderEditDetails()
	*
	* @DESC 		: -
	* @RETURN VALUE : $outputdata array
	* @PARAMETER 	: -
	...
	* @ACCESS POINT : -
	**/
	public function frameSalesOrderEditDetails($modelOutput)
    {
	...
	**/
	...
	/**
	* @METHOD NAME 	: getSalesOrderList()
	*
	* @DESC 		: TO GET THE SALES ORDER LIST  
	* @RETURN VALUE : $outputdata array
	* @PARAMETER 	: -
	* @SERVICE      : WEB
	* @ACCESS POINT : -
	**/
    ...
    {
        $this->checkRequestMethod("put"); // Check the Request Method
        $modelOutput           = $this->nativeModel->getSalesOrderList($this->currentRequestData);
	...
	/**
	* @METHOD NAME 	: getOpenSalesOrdersByItemId()
	*
	* @DESC 		: TO GET THE OPEN SALES ORDER BY ITEM ID.  
	* @RETURN VALUE : $outputdata array
	* @PARAMETER 	: -
	* @SERVICE      : WEB
	* @ACCESS POINT : -
	**/
    public function getOpenSalesOrdersByItemId()
    {
        $this->checkRequestMethod("put"); // Check the Request Method
        $modelOutput           = $this->nativeModel->getOpenSalesOrdersByItemId($this->currentRequestData);
		
		$openSalesOrderItems = array();
		foreach($modelOutput['searchResults'] as $key => $value){	
			$itemList 		= $this->nativeModel->getItemListDetails($value['id'], $this->currentRequestData);
			foreach($itemList as $ikey => $ivalue){

				$listDetailVal = array();
				$listDetailVal = array_merge(
											array("document_number"	=> $value['document_number']), 
											$ivalue,
											array("delivery_date"	=> $value['delivery_date'])
											);
				$openSalesOrderItems[] =  $listDetailVal;
			}
			
		}
		
		// FRAME OUTPUT
        $outputData['results']      = $openSalesOrderItems;
        $outputData['status']       = "SUCCESS";
	...
	public function getAnalyticsDetails()
	{
		...
		$passData['tableName']	= SALES_ORDER; 
	...
	{
		$modelOutput	= $this->nativeModel->getSalesOrderList($this->currentRequestData, 1);
		$resultsData 	= $modelOutput['searchResults'];
		$fileName		= $this->config->item('SALES_ORDER')['excel_file_name'];

		$outputData 	= processExcelData($resultsData, $fileName, $this->config->item('SALES_ORDER')['columns_list']);
		$this->output->sendResponse($outputData);
	}
	...
	public function downloadInvoice($id = '', $isMailDoc = 0)
	{
		...
		if (count($modelOutput) > 0) {
			$modelOutput 	 = $this->frameSalesOrderEditDetails($modelOutput[0]);
			...
			if (count($companyDetails) == 1) {
				...
				$invoiceProcessData['fileName']			= 'sales_order';
				$invoiceProcessData['fileHeadingName']  = 'SALES ORDER';
			...
			$outputData['message']  = lang('MSG_236'); 
		}
		$this->output->sendResponse($outputData);
	}
	...
	**/
	...
	public function proceedApprovalActivityForDocument()
	{
		...
		if (!empty($id) && is_numeric($id)) {
			...
			$getDocumentData 			= $this->nativeModel->editSalesOrder($id);
