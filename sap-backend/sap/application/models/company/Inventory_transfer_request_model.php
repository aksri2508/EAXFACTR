...
/*
* @FileName 		 : Inventory_transfer_request_model.php
* @Class  			 : Inventory_transfer_request_model
* Model Name         : Inventory_transfer_request_model
* Description        :
* Module             : company
* Actors 	         : -
...
* Features           : 
*/
class Inventory_transfer_request_model extends CI_Model
{
	public function __construct()
	{
		parent::__construct();
		$this->tableNameStr 			= 'INVENTORY_TRANSFER_REQUEST';
		$this->itemTableNameStr 		= 'INVENTORY_TRANSFER_REQUEST_ITEMS';
		$this->itemTableColumnRef	 	= 'inventory_transfer_request_id';
		$this->itemTableColumnReqRef 	= 'inventoryTransferRequestId';
	 ...
	 * @ACCESS POINT : -
	 **/
	public function saveInventoryTransferRequest($getPostData)
	{
		...
		if ($insertId > 0 && isset($getPostData['itemListArray'])) {
			...
			foreach ($getListData as $key => $value) {
				...
				$this->saveInventoryTransferRequestItems($value);
			}
		}
			// SAVE OPEN QUALITY 
	...
	/**
	 * @METHOD NAME 	: saveInventoryTransferRequestItems()
	 *
	 * @DESC 			: TO SAVE THE INVENTORY TRANSFER REQUEST ITEMS
	 * @RETURN VALUE 	: $modelOutput array
	 * @PARAMETER 		: $getPostData array
	 * @SERVICE 		: WEB
	 * @ACCESS POINT 	: -
	 **/
	public function saveInventoryTransferRequestItems($getPostData)
	{
	 ...
	 * @ACCESS POINT 	: -
	 **/
	public function updateInventoryTransferRequest($getPostData)
	{
		...
		$oldMainDetails  	 			  = $this->editInventoryTransferRequest($id);
		$oldItemDetails['itemListArray']  = $this->editInventoryTransferRequestItemList($id);
		...
		if (count($deletedItemChildIds) > 0) { // Child values
			foreach ($deletedItemChildIds as $key => $value) {
				...
				$this->deleteInventoryTransferRequestItems($passStageId);
			}
		}
		...
		if (isset($getPostData['itemListArray'])) {
			...
			foreach ($getListData as $key => $value) {
				...
				if (empty($value['id'])) { // INSERT THE RECORD 
					...
					$value['id'] = $value['id'];
					$this->updateInventoryTransferRequestItems($value);
				}
			}
		}
	...
	/**
	 * @METHOD NAME   : updateInventoryTransferRequestItems()
	 *
	 * @DESC 		  : -
	 * @RETURN VALUE  : $modelOutput array
	 * @PARAMETER 	  : $getPostData array
	 ...
	 * @ACCESS POINT  : -
	 **/
	public function updateInventoryTransferRequestItems($getPostData)
	{
	...
	/**
	 * @METHOD NAME   : editInventoryTransferRequestItemList()
	 *
	 * @DESC 		  : - 
	 * @RETURN VALUE  : $rs array
	 * @PARAMETER 	  : $getPostData array
	 ...
	 * @ACCESS POINT  : -
	 **/
	public function editInventoryTransferRequestItemList($id)
	{
	...
	/**
	 * @METHOD NAME 	: deleteInventoryTransferRequestItems()
	 *
	 * @DESC 			: -
	 * @RETURN VALUE 	: $modelOutput array
	 * @PARAMETER 		: $getPostData array
	 ...
	 * @ACCESS POINT 	: -
	 **/
	public function deleteInventoryTransferRequestItems($getPostData)
	{
	 ...
	 * @ACCESS POINT 	: -
	 **/
	public function getInventoryTransferRequestList($getPostData, $downloadFlag = '')
	{
		// Query
		$query = 'SELECT * FROM 
					(SELECT
		...
		if (isset($tableProperties['sortField'])) {
			...
			$fieldName	   = getListingParams($this->config->item('INVENTORY_TRANSFER_REQUEST')['columns_list'], $fieldName);
	...
	/**
	 * @METHOD NAME 	: getInventoryTransferRequestItemList()
	 *
	 * @DESC 			: -
	 * @RETURN VALUE 	: $rs array
	 * @PARAMETER 		: $getPostData array
	 ...
	 * @ACCESS POINT 	: -
	 **/
	public function getInventoryTransferRequestItemList($id)
	{
		$rowData = bindConfigTableValues($this->itemTableNameStr, 'EDIT', $id);
		$this->app_db->select($rowData);
		$this->app_db->from($this->itemTableName);
