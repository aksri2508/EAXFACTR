...
/*
 * @FileName 		 : sales_quote_model.php
 * @Class  			 : sales_quote_model
 * Model Name         : sales_quote_model
 * Description        :
 * Module             : company
 * Actors 	         : -
...
class Sales_quote_model extends CI_Model
{
	 ...
	 * @ACCESS POINT : -
	 **/
	public function saveSalesQuote($getPostData)
	{
		...
		if (
			($getapprovalProcessStatus['approvalStatus'] == 1) &&
	 ...
	 * @ACCESS POINT 	: -
	 **/
	public function updateSalesQuote($getPostData)
	{
		...
		$oldMainDetails = $this->editSalesQuote($id);
	...
	{
		//echo "EDITSALESQUOTE is ".$this->currentbranchId;
	 ...
	 * @ACCESS POINT 	: -
	 **/
	public function getSalesQuoteList($getPostData, $downloadFlag = '')
	{
		//echo "this->currentbranchId is ".$this->currentbranchId;
		...
		if (strtolower($this->currentEmployeeType) == 'user') {
			if (($this->hierarchyMode == 2) && $this->currentAccessControlId != 1) { // ADMIN CONDITION & RM Flow // hirarachy mode  '1 -Distribution Rules, 2 - Reporting Manager', 
		...
		if (isset($tableProperties['sortField'])) {
			...
			$fieldName = getListingParams($this->config->item('SALES_QUOTE')['columns_list'], $fieldName);
