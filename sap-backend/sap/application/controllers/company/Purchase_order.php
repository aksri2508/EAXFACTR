...
class Purchase_order extends MY_Controller
{
	...
	{
		parent::__construct();
		$this->config->load('table_config/tbl_purchase_order.php');
		$this->config->load('table_config/tbl_purchase_order_items.php');
		$this->load->model('company/purchase_order_model', 'nativeModel');
		
		$screenDetails				 = $this->config->item('SCREEN_NAMES')['PURCHASE_ORDER'];
	...
	public function updatePurchaseOrder()
	{
		...
		$outputData['status']  = "FAILURE";
		$modelOutput		   = $this->nativeModel->updatePurchaseOrder($this->currentRequestData);
		$mof = $modelOutput['flag'];

		if (1 == $modelOutput['flag']) {
			$outputData['status']       = "SUCCESS";
			$outputData['message']      = lang('MSG_178'); //'Successfully updated
		} else if (2 == $modelOutput['flag']) {
	...
	public function copyPurchaseOrder()
	{
		...
		if(count($copiedIds) == 1) {
			return $this->editPurchaseOrder($copiedIds[0]);
		}else if(count($copiedIds)>1){
			
			$getFirstCopyRecordDetails = $this->nativeModel->editPurchaseOrder($copiedIds[0]);
			
			if(count($getFirstCopyRecordDetails) > 0){
				$frameEditDetails = $this->framePurchaseOrderEditDetails($getFirstCopyRecordDetails[0]);
				...
				foreach($copiedIds as $copyKey => $copyValue){
					$getItemList 	 = $this->nativeModel->editPurchaseOrderItemList($copyValue);
					$itemListArray = array_merge($itemListArray,$this->framePurchaseOrderItemList($getItemList));
				}				
				$data 				 	= array();
	 ...
	 *
	 * @DESC 			: TO EDIT PURCHASE ORDER DETAILS 
	 * @RETURN VALUE	: $outputdata array
	 * @PARAMETER 		: -
	 * @SERVICE      	: WEB
	 * @ACCESS POINT 	: -
	...
	public function editPurchaseOrder($id = '')
	{
		...
		if (!empty($id) && is_numeric($id)) {
			...
			$modelOutput 				= $this->nativeModel->editPurchaseOrder($id);
			$getPurchaseOrderItemList 	= $this->nativeModel->editPurchaseOrderItemList($id);
			...
			if (count($modelOutput) > 0) {
				...
				$frameEditDetails 	  = $this->framePurchaseOrderEditDetails($modelOutput[0]);			
				$data 				  = array();
				$data				  = $frameEditDetails;

				// BIND THE LIST SUB ARRAY 
				$data['showAddButton']	= $showAddButton;
				$data['itemListArray'] 	= $this->framePurchaseOrderItemList($getPurchaseOrderItemList);
				$outputData['status']   = "SUCCESS";
				$outputData['results']  = $data;
			} else {
				$outputData['message'] =  lang('GLB_015');  // INVALID ID PASSED
	...
	/**
	* @METHOD NAME 	: framePurchaseOrderEditDetails()
	*
	* @DESC 		: TO FRAME THE PURCHASE ORDER EDIT DETAILS 
	* @RETURN VALUE : $outputdata array
	* @PARAMETER 	: -
	* @SERVICE      : WEB
	* @ACCESS POINT : -
	**/
	public function framePurchaseOrderEditDetails($modelOutput)
    {
	...
	/**
	* @METHOD NAME 	: framePurchaseOrderItemList()
	*
	* @DESC 		: TO FRAME THE PURCHASE ORDER ITEM LIST 
	* @RETURN VALUE : $outputdata array
	* @PARAMETER 	: -
	* @SERVICE      : WEB
	* @ACCESS POINT : -
	**/
	public function framePurchaseOrderItemList($itemList)
    {
	 ...
	 *
	 * @DESC 		    : TO GET THE PURCHASE ORDER LIST DETAILS 
	 * @RETURN VALUE    : $outputdata array
	 * @PARAMETER 	    : -
	 * @SERVICE         : WEB
	 * @ACCESS POINT    : -
	...
	{
		$this->checkRequestMethod("put"); // Check the Request Method
		$modelOutput = $this->nativeModel->getPurchaseOrderList($this->currentRequestData, 0);

		foreach ($modelOutput['searchResults'] as $key => $value) {
			$getItemList 		= $this->nativeModel->getPurchaseOrderItemList($value['id']);
	...
	public function getAnalyticsDetails()
	{
		...
		$passData['tableName']	= PURCHASE_ORDER; 
	...
	{
		$modelOutput	= $this->nativeModel->getPurchaseOrderList($this->currentRequestData, 1);
		$resultsData 	= $modelOutput['searchResults'];
		
		//printr($resultsData);exit;
		$fileName		= $this->config->item('PURCHASE_ORDER')['excel_file_name'];

		$outputData 	= processExcelData($resultsData, $fileName, $this->config->item('PURCHASE_ORDER')['columns_list']);
		$this->output->sendResponse($outputData);
	}
	 ...
	 *
	 * @DESC 		 : DOWNLOAD THE PURCHASE ORDER INVOICE
	 * @RETURN VALUE : $outputdata array
	 * @PARAMETER 	 : -
	 * @SERVICE      : WEB
	 * @ACCESS POINT : -
	...
	public function downloadInvoice($id = '', $isMailDoc = 0)
	{
		...
		$modelOutput = $this->nativeModel->editPurchaseOrder($id);
		$purchaseOrderItemList = $this->nativeModel->editPurchaseOrderItemList($id);

		if (count($modelOutput) > 0) {
            // Getting Purchase Order Info details.
			$modelOutput = $this->framePurchaseOrderEditDetails($modelOutput[0]);
			$itemList = $this->framePurchaseOrderItemList($purchaseOrderItemList);
			...
			if (count($companyDetails) == 1) {
				...
				$invoiceProcessData['fileName'] = 'purchase_order';
				$invoiceProcessData['fileHeadingName'] = 'PURCHASE ORDER';
			...
			$outputData['message']  = lang('MSG_196'); 
		}
		$this->output->sendResponse($outputData);
	}
	...
	**/
	...
	public function proceedApprovalActivityForDocument()
	{
		...
		if (!empty($id) && is_numeric($id)) {
			...
			$getDocumentData 			= $this->nativeModel->editPurchaseOrder($id);
